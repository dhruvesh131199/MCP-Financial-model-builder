...
export async function exportDcfToExcel(
  ...
  const growths = Array.isArray(inputs.revenue_growth)
    ? inputs.revenue_growth
    : Array(n).fill(inputs.revenue_growth);
  ...
  const assumptions: [string, number, string][] = [
    ...
    ["Tax Rate", inputs.tax_rate, "0.0%"],
    ["EBITDA Margin", inputs.ebitda_margin, "0.0%"],
    ["CapEx % Revenue", inputs.capex_pct, "0.0%"],
    ["ΔNWC % Revenue Growth", inputs.nwc_pct, "0.0%"],
  ...
  for (let i = 0; i < n; i++) {
    ...
    cell.value = { formula: `${colL}${revRow}*${marginRef}` };
    cell.numFmt = '#,##0.0';
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i === 0) {
      cell.value = {
        formula: `(${colL}${revRow}-${baseRevRef})*${nwcRef}`,
      };
    } else {
      const prevCol = COL(col - 1);
      cell.value = {
        formula: `(${colL}${revRow}-${prevCol}${revRow})*${nwcRef}`,
      };
    }
    cell.numFmt = '#,##0.0';
  }

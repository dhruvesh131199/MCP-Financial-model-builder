...
| 2026-07-04 | Homework + cleanup | RAG Phase 2a retrieve: `postgres_search` (pgvector top-25), `hf_rerank` (HF `BAAI/bge-reranker-v2-m3`), `retrieve_homework` CLI + two-stage HTML report; moved `hf_client` → `integrations/`; removed `embed_homework`, `huggingface_test`, orphan homework CLIs, `homework_rag` API, `RagMarkitdownPage`. |

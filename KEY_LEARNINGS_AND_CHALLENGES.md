...
| Date | Type | Summary |
|------|------|---------|
| 2026-07-09 | Dev tooling | Local MCP Inspector: `devtools/mcp-inspector/` + `scripts/inspect-mcp.sh` for testing tools/progress against `localhost:8080/mcp`; excluded from EC2 sparse checkout. Fixed `fetch_report` to use `mcp.get_context()` so progress fires after `mcp.server` module hijack. |
| 2026-07-09 | Feature | MCP progress for `fetch_report`: async tool with FastMCP `Context.report_progress` + `ctx.info`; step budget split across tickers/years; fine-grained hooks for SEC statements, 10-K ingest (MarkItDown, chunks, Postgres, HF embed batches). Progress bar needs client `progressToken`; log messages still show without it. |
...
| 2026-07-01 | MCP | Reduced tool surface: removed `set_comparative_inputs` and `run_trend_analysis` MCP tools — comparative inputs merge into `run_comparative_analysis(values=...)`; trend table stays inside `run_detailed_analysis` (rerun for refresh). |
...
| 2026-07-04 | Cleanup | Moved `hf_client` → `integrations/`; removed superseded homework (`embed_homework`, `huggingface_test`, orphan CLIs, `homework_rag` API, `RagMarkitdownPage`). Retrieve prototype not on main — tag `before-loop-engineering-retrieval`. |
| 2026-07-01 | Homework | RAG Phase 2a (superseded): in-memory cosine embed test — replaced by Postgres retrieve + rerank homework. |
